...
include('../delt.php');
echo '<head>';
echo '<link type="text/css" href="konfigurer.css" rel="stylesheet"/>';
echo '</head>';

$access=0;
if($_POST["typedpassword"]===$adminpassword_indstillingen) $access=1;	//admin indstillingen

if(!$access) {
    insertHeader("Alumneliste - Konfiguration", "Forkert kode");
	...
	<form action="konfigurer.php" method="post" name="email_body">
		...
		Send velkomst-email: <?php selector('sendEmails',$rs[0]['sendEmailToNewPersons'],array(1,0),array('Ja','Nej')); ?><br>
	...
	<form action="konfigurer.php" method="post" name="emails">
		...
		selector('email_to_delete','',$emails,$emails);
		?><br>
	...
	<form action="konfigurer.php" method="post" name="email_body">
		...
		Send email når netværk lukkes: <?php selector('enabled1',$rs[0]['enabled'],array(1,0),array('Ja','Nej')); ?><br>
		<br>Email når netværk åbnes:<br>
		<textarea id="body2" name="body2" rows="10" cols="55"><?php echo $rs[1]['body']; ?></textarea><br>
		Send email når netværk åbnes: <?php selector('enabled2',$rs[1]['enabled'],array(1,0),array('Ja','Nej')); ?><br>
		<input type="submit" value="Foretag ændringer">
	</form>
	</div>
    <?php
