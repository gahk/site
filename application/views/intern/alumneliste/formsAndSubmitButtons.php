...
//echo '<form id="form_addperson" action="liste.php" method="post">';
...
foreach($newPersonVariables as $varName) {
	...
	if($varName != "name") {
		...
		echo '<label style="margin-bottom: 0px" for="'.$varName.'">'.translate($varName).': </label>';
		...
		if($varName==="birthday") {
			...
			selector("moveInMonth", date("m",time()+15*24*3600), numberArray(1,12),array_values($monthName), null);

			selector("moveInYear", date("Y",time()+15*24*3600), numberArray($year-5,$year+1), numberArray($year-5,$year+1), null);

		

		} elseif($varName==="room") {

			selector("room",0,$room_number,$roomDescription, null);

		

		} elseif($varName==="fylgje") {

			selector("fylgje","",reduceArrayArray($alumner,"name"),reduceArrayArray($alumner,"name"), null);
			...
			selector($varName,"",reduceArrayArray($rs,$varName),reduceArrayArray($rs,$varName), null);
...
echo '<label style="margin-bottom: 0px" for="alumne_ID">Alumne: </label>'; selector("alumne_ID",0,reduceArrayArray($rs,"ID"),reduceArrayArray($rs,"name"), null); echo '<br>';

echo '<label style="margin-bottom: 0px" for="room2">Værelse: </label>'; selector("room2",0,$room_number,$roomDescription, null); echo '<br>';
...
selector("alumneID",0,reduceArrayArray($alumner,"alumne_ID"),reduceArrayArray($alumner,"name"), null); //PROBLEM HERE? CHECK
...
echo "Eksisterende alumne: "; selector("alumne_ID",0,reduceArrayArray($rs,"ID"),reduceArrayArray($rs,"name"), null); echo '<br>';

echo '<label style="margin-bottom: 0px" for="newFirstName">Fornavn: </label>';

echo '<input type="text" name="newFirstName" id="newFirstName" size="30"/><br>';

echo '<label style="margin-bottom: 0px" for="newLastName">Efternavn: </label>';

echo '<input type="text" name="newLastName" id="newLastName" size="30"/><br>';

echo '<label style="margin-bottom: 0px" for="newNameOK" style="float: left; width: 150px;">Godkend navneændring: </label>';
...
selector('studyToDelete',0,reduceArrayArray($rs,"ID"),reduceArrayArray($rs,"study"), null);
...
if(!in_array($month+1,reduceArrayArray($rs,"monthNumber"))) {
	...
	echo '<label style="margin-bottom: 0px" for="confirmCopy" style="float: left; width: 170px;">Godkend kopiering af liste: </label>';
...
echo '<label style="margin-bottom: 0px" for="confirmDelete" style="float: left; width: 170px;">Godkend sletning af liste: </label>';
